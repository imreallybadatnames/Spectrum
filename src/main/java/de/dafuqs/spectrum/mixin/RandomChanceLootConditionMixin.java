...
public abstract class RandomChanceLootConditionMixin {
	...
	@Shadow @Final private LootNumberProvider chance;
	...
	public boolean spectrum$applyRareLootEnchantment(boolean original, LootContext context) {
		...
		if (!original) {
			original = context.getRandom().nextFloat() < CloversFavorHelper.rollChance(chance.nextFloat(context), context.get(LootContextParameters.ATTACKING_ENTITY));
		}
		return original;
	}
