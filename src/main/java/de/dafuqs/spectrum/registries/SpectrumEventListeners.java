...
public class SpectrumEventListeners {
	...
	public static void register() {
		...
		CrossbowShootingCallback.register((world, shooter, hand, projectileEntity) -> {
			var crossbowStack = projectileEntity.getWeaponStack();
			if (crossbowStack != null && crossbowStack.getItem() instanceof GlassCrestCrossbowItem && GlassCrestCrossbowItem.isOvercharged(crossbowStack)) {
				if (!world.isClient) { // only fired on the client, but making sure mods aren't doing anything weird
					...
					if (GlassCrestCrossbowItem.getOvercharge(crossbowStack) > 0.99F) {
					...
					GlassCrestCrossbowItem.unOvercharge(crossbowStack);
				}
			}
		});
