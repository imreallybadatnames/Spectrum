...
public class ProjectorBlockEntityRenderer implements BlockEntityRenderer<ProjectorBlockEntity> {
	...
	public void render(ProjectorBlockEntity entity, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay) {
		...
		renderRing(matrices, buffer, entity.scaling, 0.75F, overlay);
		matrices.pop();
	}

	//Yes I did in fact just steal this from pastel nodes :3
	private void renderRing(MatrixStack matrices, VertexConsumer vertices, float scale, float alpha, int overlay) {
		var size = scale / 16F;
	...
	private void renderSide(Matrix4f model, VertexConsumer vertices, float alpha, float u, float v, float x1, float x2, float z1, float z2, int overlay) {
		...
		vertices.vertex(model, x1, z2, 0).color(1F, 1F, 1F, alpha).texture(u1, v1).overlay(overlay).light(LightmapTextureManager.MAX_LIGHT_COORDINATE).normal(0, 1, 0);
		vertices.vertex(model, x2, z2, 0).color(1F, 1F, 1F, alpha).texture(u2, v1).overlay(overlay).light(LightmapTextureManager.MAX_LIGHT_COORDINATE).normal(0, 1, 0);
		vertices.vertex(model, x2, z1, 0).color(1F, 1F, 1F, alpha).texture(u2, v2).overlay(overlay).light(LightmapTextureManager.MAX_LIGHT_COORDINATE).normal(0, 1, 0);
		vertices.vertex(model, x1, z1, 0).color(1F, 1F, 1F, alpha).texture(u1, v2).overlay(overlay).light(LightmapTextureManager.MAX_LIGHT_COORDINATE).normal(0, 1, 0);
	}
}
