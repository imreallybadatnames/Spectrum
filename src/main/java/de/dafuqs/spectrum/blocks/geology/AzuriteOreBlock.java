...
public class AzuriteOreBlock extends CloakedOreBlock {
    ...
    public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
        ...
        if (world.isClient() && !entity.bypassesSteppingEffects() && world.random.nextInt(3) == 0 && entity instanceof PlayerEntity player && this.isVisibleTo(player)) {
			ParticleHelper.playParticleAroundBlockSides(world, SpectrumParticleTypes.AZURE_MOTE_SMALL, pos, new Direction[]{Direction.UP}, 1, Vec3d.ZERO);
        }
    }
    ...
    public BlockState onBreak(World world, BlockPos pos, BlockState state, PlayerEntity player) {
        ...
        if (world.isClient() && this.isVisibleTo(player)) {
			...
			ParticleHelper.playParticleAroundBlockSides(world, SpectrumParticleTypes.AZURE_MOTE_SMALL, pos, Direction.values(), 3, Vec3d.ZERO);
        }
    ...
    public void onBlockBreakStart(BlockState state, World world, BlockPos pos, PlayerEntity player) {
        ...
        if (world.isClient() && this.isVisibleTo(player)) {
			ParticleHelper.playParticleAroundBlockSides(world, SpectrumParticleTypes.AZURE_MOTE, pos, Direction.values(), 1, Vec3d.ZERO);
        }
    }
    ...
    public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
        ...
        if (this.isVisibleTo(MinecraftClient.getInstance().player)) {
			...
			ParticleHelper.playParticleAroundBlockSides(world, SpectrumParticleTypes.AZURE_MOTE, pos, Direction.values(), random.nextBetween(1, 3), Vec3d.ZERO);
